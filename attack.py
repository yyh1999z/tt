...
def attack(args, attack_item_num=2, model_root=None):
    ...
    for user in train.keys():
        ...
        #     print(user,':',seqs)
        for i in seqs:
            item_counter[i] += 1    #计算项目被多少用户打分
   
    item_popularity = []
    for i in item_counter.keys():
        item_popularity.append((item_counter[i], i)) #打分次数和项目
    item_popularity.sort(reverse=True)
    
    print('***** Untargeted Substitution Attacks *****')
    attacker = AdversarialAttacker(args, model, test_loader,max_predictions=40,mask_prob = 0.2)
