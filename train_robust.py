...
def train(args, export_root=None, resume=True):
    ...
    if export_root == None:
        export_root = 'experiments/' + str(args.defense_method) + '/' + str(args.model_code) + '/' + str(args.dataset_code)
    ...
    if args.model_code == 'locker':
        trainer = BERTAdvTrainer(args, model, train_loader, val_loader, test_loader, export_root)
    elif args.model_code == 'bert':
        trainer = BERTAdvTrainer(args, model, train_loader, val_loader, test_loader, export_root)
    elif args.model_code == 'sas':
        trainer = SASAdvTrainer(args, model, train_loader, val_loader, test_loader, export_root)
    elif args.model_code == 'narm':
        trainer = RNNAdvTrainer(args, model, train_loader, val_loader, test_loader, export_root)
