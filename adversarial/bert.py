...
class BERTAdvTrainer(metaclass=ABCMeta):
    def __init__(self, args, model, train_loader,val_loader, test_loader, export_root):
        ...
        if self.args.defense_method == 'dirichlet':
            ...
            self.train_loader.collate_fn = self.expand_and_sample_dirichlet ##用更新的原始样本进行训练
            # ##
            # self.MYTrain_loader.collate_fn = self.expand_and_sample_dirichlet
            # ##
    ...
    def build_joint_synonym_matrix(self, min_items=2, max_items=50):#构建同义词矩阵
    ...
    def build_1_order_synonym_dict(self, min_items=2, max_items=25):#构建一跳字典
        self.synonym_dict_1_order = {}
        self.model.eval()
        with torch.no_grad():
            cos = nn.CosineSimilarity(dim=-1, eps=1e-6)
            item_embeddings = self.model.embedding.token.weight.detach()[1:-1]#项目嵌入
            for i in range(len(item_embeddings)):
                ...
                indices = (scores >= self.args.min_cos_sim).nonzero(as_tuple=False)#找出大于min_cos_sim的项
    ...
    def sample_dirichlet_synonyms_fast(self, item, idx, require_coeff=True):
        ...
        if require_coeff:
            coeffs = self.dirichlet_cache[(num_synonym_1_order, \
                num_synonym_2_order)][idx%(self.args.cache_size**2)] #分布概率
        return num_synonym, synonyms, coeffs

    def expand_and_sample_dirichlet(self, batch):
        seqs, labels = [], []
        for i in range(len(batch)):
            seq = batch[i][0] #每个用户的序列
            seq_expanded = F.one_hot(seq,
                num_classes=self.model.embedding.token.weight.size(0)).float() #onehot 编码么个序列
            item_positions = ((seq != 0) * (seq != self.mask_token)#删除0和mask后获得位置信息
                ).nonzero().squeeze().view(-1)
            if len(item_positions) == 0:
                seqs.append(seq_expanded)
                labels.append(batch[i][1])
                continue
            min_sample = max(1, int(self.args.substitution_ratio*len(item_positions)))#替换的数量 长度*0.5
            swap_positions = np.random.choice(item_positions, min_sample, replace=False)#随机替换顺序
    ...
    def train_one_epoch(self, epoch, accum_iter):
        ...
        for batch_idx, batch in enumerate(tqdm_dataloader):
            ...
            if self.args.defense_method == 'advtrain':
                ...
                switch_indices = (switch_indices * (seq_before != 0) * (seq_before != self.mask_token)).float().unsqueeze(-1)
